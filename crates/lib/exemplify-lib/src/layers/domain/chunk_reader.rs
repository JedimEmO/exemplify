...
//This chunk has no explicit end
        ...
        let reader_a = stringreader::StringReader::new(content_a);
        let parser_settings = ParserSettings { start_token: "##exemplify-start##".into(), end_token: "##exemplify-end##".into() };

        let mut chunk_reader = ChunkReader::new(reader_a, parser_settings);

        while let Some(item) = chunk_reader.next().await {
            match item {
                Ok(chunks) => {
                    println!("got {} chunks", chunks.len());

                    for chunk in chunks {
                        println!("{}", chunk.content.join("\n"));
                    }
                }
                Err(e) => panic!(e)
            }
